...
(function () {
  ...
  webPrime.createdCallback = function() {
    ...
    if(this.config.disableWebGL || !this.config.runningWebGL)  
        this.gameVariables.drawer = new Drawer({ "canvas": this.canvas,    "skin" : this.gameVariables.skin, "webprime": this });
    else
        this.gameVariables.drawer = new PrimeGL.Drawer({ "canvas": this.canvas,    "skin" : this.gameVariables.skin, "gl" : this.gl, "webprime": this });
  ...
  webPrime._internalAnimate = function(a) {
    if( a.config.highSpeedAnimation)
        HighSpeedRequest(function() { a._internalAnimate(a); });
    else

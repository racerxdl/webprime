      ...
      switch (loaderType) {
        ...
        case PrimeConst.TypeJPAKNX:
          if (this.config.fileName !== undefined && this.config.songLevel !== undefined && this.config.songMode !== undefined)   {
            this.jpakloader = new JPAK.jpakloader({"file" : this.config.fileName});
            this.jpakloader.onload = function()   {
              ...
              var soundFile   = (_this.config.supportedCodecs.audio.indexOf("mp3") > -1) ? 
                                this.GetFileURL("/MUSIC.MP3", "audio/mpeg") : 
                                this.GetFileURL("/MUSIC.OGG", "audio/ogg");

              var imageFile   = this.GetFileURL("/TITLE.JPG", "image/jpeg");

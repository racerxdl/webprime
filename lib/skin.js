...
(function () {
  ...
  var F2Skin = function(parameters)  {
    ...
    var comboLoader = new JPAK.Loader({"file":PrimeConst.ComboDataPack});
    comboLoader.load().then(function() {

      function makeLoadHandler() {
        return function () {
          var z;
          if (!_skin.webprime.config.runningWebGL) {
            // Combo Name
            _skin.Combo.name = ImageTools.cropImage(this, _skin.webprime.gameVariables.comboNameX, _skin.webprime.gameVariables.comboNameY, _skin.webprime.gameVariables.comboNameWidth, _skin.webprime.gameVariables.comboNameHeight);

            // From 0 to 6 is on the first line
            for (z=0;z<7;z++)
             _skin.Combo.numbers[z] = ImageTools.cropImage(this, _skin.webprime.gameVariables.comboNumberWidth*z+z,0,_skin.webprime.gameVariables.comboNumberWidth,_skin.webprime.gameVariables.comboNumberHeight);

            // From 7 to 9 is on the second line
            for (z=0;z<3;z++)
             _skin.Combo.numbers[z+7] = ImageTools.cropImage(this, _skin.webprime.gameVariables.comboNumberWidth*z+z,_skin.webprime.gameVariables.comboNumberHeight,_skin.webprime.gameVariables.comboNumberWidth,_skin.webprime.gameVariables.comboNumberHeight);
          } else {
            _skin.Combo.numbers.texture = _skin.gl.createTexture();
            _skin.gl.bindTexture(_skin.gl.TEXTURE_2D, _skin.Combo.numbers.texture);
            ...
            _skin.Combo.numbers.texture.width = PrimeGL.Tools.nextHighestPowerOfTwo(this.width);
            _skin.Combo.numbers.texture.height = PrimeGL.Tools.nextHighestPowerOfTwo(this.height);
            _skin.Combo.numbers.coords = {};
            _skin.Combo.name.texture = _skin.Combo.numbers.texture;
            _skin.Combo.name.coords = {
              x: (_skin.webprime.gameVariables.comboNameX)                                                / this.width,
              y: (_skin.webprime.gameVariables.comboNameY)                                                / this.width,
              u: (_skin.webprime.gameVariables.comboNameX + _skin.webprime.gameVariables.comboNameWidth)  / this.width,
              v: (_skin.webprime.gameVariables.comboNameY + _skin.webprime.gameVariables.comboNameHeight) / this.width
            };
            // From 0 to 6 is on the first line
            for (z=0;z<7;z++)
              _skin.Combo.numbers.coords[z] = {
                x: (_skin.webprime.gameVariables.comboNumberWidth*z+z)      / this.width,
                y: 0,
                u: (_skin.webprime.gameVariables.comboNumberWidth*(1+z)+z)  / this.width,
                v: (_skin.webprime.gameVariables.comboNumberHeight)         / this.width
              };

            // From 7 to 9 is on the second line
            for (z=0;z<3;z++)
              _skin.Combo.numbers.coords[z+7] = {
                x: (_skin.webprime.gameVariables.comboNumberWidth*z+z)      / this.width,
                y: (_skin.webprime.gameVariables.comboNumberHeight)         / this.width,
                u: (_skin.webprime.gameVariables.comboNumberWidth*(1+z)+z)  / this.width,
                v: (_skin.webprime.gameVariables.comboNumberHeight*2)       / this.width
              };
          }
      ...
      comboLoader.getFileURL("/"+(_skin.webprime.graphicalMode.toUpperCase())+"/"+PrimeConst.ComboImage,PrimeConst.mimepng).then(function(url) {
        var tmpImg = new Image();
        tmpImg.onload = makeLoadHandler();
  ...
  F2Skin.prototype.GetComboImage = function (number) {
    return this.Combo.numbers[number];
  };

  F2Skin.prototype.GetComboNameImage = function() {
  ...
  F2Skin.prototype.GLGetComboImage = function (number) {
    var coord = this.Combo.numbers.coords[number];
    return {
      texture: this.Combo.numbers.texture,
      x: coord.x,
      y: coord.y,
      u: coord.u,
      v: coord.v
  ...
  F2Skin.prototype.GLGetComboNameImage = function (number) {
    var coord = this.Combo.name.coords;
    return {
      texture: this.Combo.name.texture,
      x: coord.x,
      y: coord.y,
      u: coord.u,
      v: coord.v

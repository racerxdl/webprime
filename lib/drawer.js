...
(function () {
   ...
   */
  CanvasDrawer.prototype.DrawCombo =   function()   {
    var comboString = ""+this.webprime.gameVariables.currentCombo;
    var ctx = this.SceneLayers[3].GetContext();
    ...
    if (comboString !== "0") {
      ...
      for(var i=0;i<comboString.length;i++) {
        img = this.skin.GetComboImage(parseInt(comboString[i]));
        ctx.drawImage(img, numberScreenX + numberWidth * i, numberScreenY, numberWidth, numberHeight);

        if (this.webprime.config.drawAnchors)   {
          ctx.save();
          ctx.beginPath();
          ctx.arc(numberScreenX + numberWidth / 2, numberScreenY + numberHeight / 2, 4, 0, 2 * Math.PI, false);
          ctx.fillStyle = 'red';
          ctx.fill();
          ctx.lineWidth = 2;
          ctx.strokeStyle = '#003300';
      ...
      // Combo Name
      img = this.skin.GetComboNameImage();
      ctx.drawImage(img, nameScreenX, nameScreenY, comboNameWidth, comboNameHeight);

      if (this.webprime.config.drawAnchors)   {
        ctx.save();
        ctx.beginPath();
        ctx.arc(nameScreenX + comboNameWidth / 2, nameScreenY + comboNumberHeight / 2, 4, 0, 2 * Math.PI, false);
        ctx.fillStyle = 'red';
        ctx.fill();
        ctx.lineWidth = 2;
        ctx.strokeStyle = '#003300';

...
(function () {
   ...
   */
  var AnimatedObject = function (parameters) {
   ...
   */
  AnimatedObject.prototype.GLUpdate  = function()  {
    if (this.NeedsRedraw)    {
      var data = PrimeGL.Tools.genSprite(this.x,this.y,this.coord[2]-this.coord[0],this.coord[3]-this.coord[1],this.coord[0]*this.image.XStep,this.coord[1]*this.image.YStep,this.coord[2]*this.image.XStep,this.coord[3]*this.image.YStep,1,0,this.image.flip, this.webprime);
   ...
   */
  AnimatedObject.prototype.CheckLife = function(timeDelta)    {
    if (!this.infinite)
   ...
   */
  AnimatedObject.prototype.Ressurect   = function(lifetime)    {
   ...
   */
  AnimatedObject.prototype.SetX        = function(x)   {
    if (!this.webprime.config.subPixelRender)  
        x = x >> 0;
    if (x != this.x) {
      this.x = x;
   ...
   */
  AnimatedObject.prototype.SetY        = function(y)   {
    if (!this.webprime.config.subPixelRender)  
      y = y >> 0;
    if (y != this.y) {
      this.y = y;
   ...
   */
  AnimatedObject.prototype.SetScale    = function(x,y) {
    if (!this.webprime.config.subPixelRender)  {
      x = x >> 0;
      y = y >> 0;
    }
   ...
   */
  AnimatedObject.prototype.SetScaleX   = function(x)   {
    if (!this.webprime.config.subPixelRender)  
      x = x >> 0;
    if (x != this.scale.x)   {
      this.scale.x = x;
   ...
   */
  AnimatedObject.prototype.SetScaleY   = function(y)   {
    if (!this.webprime.config.subPixelRender)  
      y = y >> 0;
    if (y != this.scale.y)   {
      this.scale.y = y;
   ...
   */
  AnimatedObject.prototype.SetVisible  = function(visible) {
   ...
   */
  AnimatedObject.prototype.Clone = function()  {
    return new AnimatedObject({"opacity" : this.opacity, "x" : this.x, "y" : this.y, "scale" : { "x" : this.scale.x, "y" : this.scale.y}, "rotation" : this.rotation, "layer" : this.layer, "image" : this.image, "infinite": this.infinite, "lifetime" : this.lifetime, "visible": this.visible, "Update" : this.Update, "gl" : this.gl, "coord" : this.coord, "webprime": this.webprime}); 
  };
   ...
   */
  var FrameObject = function(parameters) {
    this.opacity    = parameters.opacity !== undefined ? parameters.opacity : 1;
    this.x          = parameters.x          || 0;
    this.y          = parameters.y          || 0;
    this.scale      = parameters.scale      || {x:1,y:1};
    ...
    this.NeedsRedraw = true;
    PrimeLog.d("Created new FrameObject with id "+this.id);
  };

  FrameObject.prototype = new AnimatedObject();   //  The SuperClass
  FrameObject.prototype.constructor = FrameObject;

  /*
   *  Updates the frame given timeDelta
   */
  FrameObject.prototype.Update = function(timeDelta)   {
    if (this.Running)    {
   ...
   */
  FrameObject.prototype.Start  =   function(beat)  {
    if (beat !== undefined)  {
   ...
   *  Stops the animation
   */
  FrameObject.prototype.Stop   =  function()   {
    this.CurrentFrame = 0;
    this.visible = false;
    this.Running = false;
  };

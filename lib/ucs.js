    ...
    while(i<lineslength)    {
      if (lines[i][0] === ":")  {
        ...
        var note = lines[i].split(""),
            lennote = note.length,
            n = 0,
            row = new StepRow({"rowbeat" : beat, "Noteskin": CurSplit.Noteskin, "mbbeat" : mbbeat, "mysteryblock": CurSplit.mysteryblock, "rowtime" : time, "webprime": webprime}),
            addrow = false;
        while(n<lennote)    {
          if (note[n] != "\r") {
            addrow |= (note[n]!=".");
        ...
        mbbeat += CurSplit.mysteryblock * CurSplit.beatsplit;

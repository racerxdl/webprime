...
(function () {
  ...
  var UCSParser = function ( webprime, UCSText ) {
    ...
    while(i<lineslength)    {
      if(lines[i][0] === ":")  {
        ...
        while(n<lennote)    {
          if(note[n] != "\r") {
            addrow |= (note[n]!=".");
            if(note[n] in UCSData)
              row.AddNote(new StepNote({"type" : UCSData[note[n]]}));
            else{
              PrimeLog.l("Note type not know: "+note[n]+" code  "+note[n].charCodeAt(0));
              row.AddNote(new StepNote({"type" : UCSData["."]}));
            }
          }
          ++n;
        }

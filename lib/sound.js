...
(function () {
  ...
  var SoundManager = function (webprime) {
    ...
    this.Bomb = new SoundPlayer({"filename":PrimeConst.BombAudio, "reset":true, "buildnew" : true});
  };
  ...
  var SoundPlayer = function (parameters) {
    ...
    PrimeLog.d("Creating WebPrime::AudioPlayer with "+this.filename);
  ...
  SoundPlayer.prototype.Play = function()  {
    var _this = this;
    PrimeLog.d("WebPrime::AudioPlayer("+this.idname+").Play()");
    if (this.webprime.config.enableSound)   {
      if (this.buildnew)   {
        var a,b;
        b=new Date();
  ...
  SoundPlayer.prototype.Pause = function() {
    PrimeLog.d("WebPrime::AudioPlayer("+this.idname+").Pause()");
    clearTimeout(this.playtimeout);

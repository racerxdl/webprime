    ...
    for (i=0;i<numsplits;i++) {
      ...
      for (var blk=0;blk<numblocks;blk++)  {
        ...
        if (SelectedBlock != blk)   {
          ...
          LastScrollFactor = sf;
          PrimeLog.d("NX20(Block) Block("+blk+"): Adding Scroll Factor Change");
          ...
          if (Math.round(steptime*1000 + PrimeConst.StepSoundOffset*1000) != Math.round(time*1000*1000))    {
            ...
            NX20Data.AddWarp({ "WarpBeat" : beat ,  "Start" : (steptime/1000), "WarpTime" : time, "WarpDelta" : time - (steptime/1000) });
          }                    
          /*  Parse Notes */
          for (var nr=0; nr<numrows;nr++) {
            ...
            if (rowd[0] == PrimeConst.NX20.NoteTypeRow) {
              ...
              row = new StepRow({"rowbeat" : beat, "rowtime" : time, "mysteryblock" : CurSplit.mysteryblock, "webprime": webprime});
